...
void main() {
  ...
  group('ToolbarConfigNotifier', () {
    ...
    test('toggleToolVisibility works and persists', () async {
      ...
      final config = container.read(toolbarConfigProvider);
      final eraserConfig =
          config.tools.firstWhere((t) => t.toolType == ToolType.pixelEraser);
    ...
    test('reorderTools works correctly', () async {
      ...
      final initialFirst =
          container.read(toolbarConfigProvider).sortedTools.first.toolType;
    ...
    test('toggleQuickAccess works', () async {
      ...
      final notifier = container.read(toolbarConfigProvider.notifier);
      final initialValue =
          container.read(toolbarConfigProvider).showQuickAccess;
    ...
    test('loads saved config on init', () async {
      ...
      final shapesConfig =
          config.tools.firstWhere((t) => t.toolType == ToolType.shapes);
    ...
    test('updateConfig replaces entire config', () async {
      ...
      final highlighterConfig =
          config.tools.firstWhere((t) => t.toolType == ToolType.highlighter);
  ...
  group('visibleToolsProvider', () {
    test('returns only visible tools', () async {
      ...
      expect(
          visibleTools.any((t) => t.toolType == ToolType.pixelEraser), isFalse);
  ...
  group('isToolVisibleProvider', () {
    test('returns correct visibility for a tool', () async {
      ...
      expect(
          container.read(isToolVisibleProvider(ToolType.pixelEraser)), isTrue);

      // Toggle visibility
      await notifier.toggleToolVisibility(ToolType.pixelEraser);

      expect(
          container.read(isToolVisibleProvider(ToolType.pixelEraser)), isFalse);
      expect(
          container.read(isToolVisibleProvider(ToolType.ballpointPen)), isTrue);
    });
    ...
    test('returns default for unknown tool type', () {
      ...
      final isVisible =
          container.read(isToolVisibleProvider(ToolType.ballpointPen));
